...
def create_room(
    ...
    *,
    ...
    # Turner's Create Study Room preferences. Applied through an explicit
    # allow-list so the dialog can grow a field without this becoming a
    # setattr loop over whatever the client happened to send.
    for field in ("setting", "music_enabled", "chat_enabled", "focus_minutes"):
        if preferences and preferences.get(field) is not None:
            setattr(room, field, preferences[field])
