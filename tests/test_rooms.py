...
class TestHostedRooms:
    ...
    def test_turners_room_preferences_survive_the_refactor(self, api):
        """The Create Study Room dialog on `development` sends these four.
        They came from a different branch than the theme/capacity fields, so
        there is a test making sure this refactor did not quietly drop them."""
        response = api.post(
            "/api/rooms",
            json={
                "name": "Campsite",
        ...
        body = response.get_json()
        assert response.status_code == 201
        assert body["setting"] == "campsite"
        assert body["music_enabled"] is False
        assert body["chat_enabled"] is True
        assert body["focus_minutes"] == 25

    def test_room_preferences_have_defaults(self, api):
        body = api.post("/api/rooms", json={"name": "Plain"}).get_json()

        assert body["setting"] == "campsite"
        assert body["music_enabled"] is True
        assert body["focus_minutes"] == 50

    def test_an_absurd_focus_length_is_refused(self, api):
        """focus_minutes reaches a NOT NULL integer column, so it is
        validated rather than passed through."""
        response = api.post("/api/rooms", json={"name": "Marathon", "focus_minutes": 10_000})

        assert response.status_code == 400
        assert "focus_minutes" in response.get_json()["details"]["fields"]
